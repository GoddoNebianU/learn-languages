...
- root `layout.tsx` 用 `<main className="bg-primary-500 min-h-[var(--page-min-h)]">` 包装 `{children}`, 确保所有路由 (含 `loading.tsx`) 继承动态主题色, 切页时无白屏闪烁。`globals.css` 的 `body`/`html` 同样用 `var(--color-primary-500)` (跟 main 同色, 消除 overscroll/亚像素 gap 的色差)。`--page-min-h` = `calc(100vh - var(--navbar-h))`, `--navbar-h` = `calc(var(--spacing) * 16)` 跟 Tailwind `h-16` 同步 (compact 模式 navbar 缩到 48px, 不能硬编码 64px)
