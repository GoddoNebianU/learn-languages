...
└── [deck_id]/
    ...
    ├── InDeck.tsx           # Client: 牌组详情 (305 行) - 卡片列表, 拖拽排序, 批量删除, 删除牌组
    ├── CardItem.tsx         # Client: 卡片条目 (149 行) - 拖拽手柄, 正反面展示, 隐藏/显示, 删除确认, 编辑入口
    ├── AddCardModal.tsx     # Client: 添加卡片 (275 行) - 多释义动态表单, 卡片类型选择, IPA
    └── EditCardModal.tsx    # Client: 编辑卡片 (215 行) - 预填表单, meanings 动态增删
```
...
| 任务               | 位置                              | 备注                                              |
| ------------------ | --------------------------------- | ------------------------------------------------- |
| 牌组 CRUD          | `src/modules/deck/deck-action.ts` | 14 个 actions, 386 行 repository                  |
| 卡片 CRUD + 排序   | `src/modules/card/card-action.ts` | 9 个 actions, 包 reorderCards, hidden 字段        |
...
| 删除确认           | `CardItem.tsx` / `InDeck.tsx`     | 使用 DS Modal 组件                               |
| 卡片隐藏/显示      | `CardItem.tsx` EyeOff/Eye 按钮    | `actionUpdateCard({hidden})`, 隐藏卡片不出现在记忆页面 |
...
- CardItem 的删除确认使用 DS Modal 组件
- 卡片支持 `hidden` 字段, 隐藏后不出现在记忆页面; CardItem 有 EyeOff/Eye 切换按钮, 隐藏卡片半透明显示
...
├── NormalMode.tsx           # Client (250 行): 输入表单 + URL 同步 + 结果展示 + 存卡
├── ReadingMode.tsx          # Client (393 行): 逐词输入查词 + 查已有卡片 + 增删改卡
├── DictionaryEntry.tsx      # Client (40 行): 单词条展示 (音标/词性/释义/例句)
├── SpeakButton.tsx          # Client: 例句 TTS 播放按钮 (Volume2 图标)
