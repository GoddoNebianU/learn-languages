...
Next.js 16 (App Router) / React 19 / TypeScript / Tailwind CSS v4 / Prisma 7 / PostgreSQL / better-auth / next-intl (9 语言) / OpenAI 兼容 LLM / 自部署 TTS
...
共享：`llm.ts`（OpenAI 兼容 LLM 客户端）、`tts.ts`（primary TTS 服务）。
