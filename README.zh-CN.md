...
环境变量通过 `src/lib/env.ts` (Zod) 在启动时校验。必填变量（`DATABASE_URL`、`BETTER_AUTH_SECRET`）缺失时会立即崩溃。SMTP 在多用户模式下必填，单用户模式下可选。可选 API Key（`LLM_*`、`DASHSCORE_API_KEY`）在首次使用时校验。服务配置（LLM、TTS、SMTP）和功能开关存储在数据库的 `system_config` 表中。详见 `.env.example`。
